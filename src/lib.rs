...
pub mod bevy {
    ...
    fn refresh_login(
        ...
        commands.add(
            ...
            .then(asyn!(_state, result=>{
                // todo serde json di shit
                // todo errors
                println!("RES: {:?}",result.unwrap().text());
            }))
        );

        // insert idToken resource
        commands.insert_resource(BevyFirebaseIdToken("".into()));
    }
}
